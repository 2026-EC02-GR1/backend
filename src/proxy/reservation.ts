 ...
 */
const RESERVATION_SERVICE_URL = process.env.RESERVATION_SERVICE_URL ?? "http://localhost:3001"
...
async function forward(req: Request, res: Response) {
	...
	try {
		...
		res.status(502).json({ code: "service_unavailable", message: "Reservation service is unreachable" })
		return
	}
