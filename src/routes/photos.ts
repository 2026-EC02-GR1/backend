...
photosRouter.post("/", authenticate, upload.single("file"), async (req, res) => {
	...
	res.status(201).json({ id: photo.id, storage_url: photo.storage_url, order: photo.order })
})
...
photosRouter.patch("/reorder", authenticate, validate(reorderSchema), async (req, res) => {
	...
	if (ids.length !== existingIds.size || ids.some((id) => !existingIds.has(id))) {
		throw new DomainError("validation_error", "order must contain exactly all photo IDs for this property")
	}

	await Promise.all(
		ids.map((id, index) =>
			prisma.propertyPhoto.update({ where: { id }, data: { order: index + 1 } }),
		),
	)
