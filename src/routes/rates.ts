...
import { z } from "zod";
import type { Prisma, Rate } from "../../generated/prisma/client";
...
type DiscountRuleWithRate = Prisma.DiscountRuleGetPayload<{
	include: { rate: true };
}>;
...
ratesRouter.patch(
	...
	async (req, res) => {
		const { property_id, rate_id } = req.params as {
			property_id: string;
			rate_id: string;
		};
		await requireProperty(property_id, req.manager);
		...
		const others = await prisma.rate.findMany({
			where: { property_id, NOT: { id: rate_id } },
		});
...
ratesRouter.post(
	"/:rate_id/discount-rules",
	authenticate,
	validate(discountRuleCreateSchema),
	async (req, res) => {
		const { property_id, rate_id } = req.params as {
			property_id: string;
			rate_id: string;
...
ratesRouter.delete(
	"/:rate_id/discount-rules/:rule_id",
	authenticate,
	async (req, res) => {
		...
		if (
			!rule ||
			rule.rate.property_id !== property_id ||
			rule.rate_id !== rate_id
		) {
