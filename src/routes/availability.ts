...
import type { BlockedPeriod, HoldSlot, Rate, DiscountRule } from "../../generated/prisma/client"
...
function toPeriodInput(b: BlockedPeriod) {
	return {
		...
		source: b.source as "manual" | "ical_airbnb" | "ical_booking" | "booking_confirmed",
	}
}
...
availabilityRouter.get("/", authenticate, async (req, res) => {
	const { property_id } = req.params
	const query = availabilityQuerySchema.safeParse(req.query)
	if (!query.success) {
		const fields = query.error.issues.map((i) => ({ field: i.path.join("."), message: i.message }))
		res.status(422).json({ code: "validation_error", message: "Validation failed", fields })
		return
	}

	const property = await prisma.property.findUnique({ where: { id: property_id } })
	if (!property || property.deleted_at !== null) throw new DomainError("not_found", "Property not found")
	if (property.user_id !== req.manager.id) throw new DomainError("forbidden", "You do not have access to this property")
	...
	res.json(computeAvailability(start_date, end_date, blocks.map(toPeriodInput), holds.map(toHoldInput)))
})

// ── PATCH /properties/:property_id/availability ────────────────────────────
availabilityRouter.patch("/", authenticate, validate(availabilityPatchSchema), async (req, res) => {
	const { property_id } = req.params
	const property = await prisma.property.findUnique({ where: { id: property_id } })
	if (!property || property.deleted_at !== null) throw new DomainError("not_found", "Property not found")
	if (property.user_id !== req.manager.id) throw new DomainError("forbidden", "You do not have access to this property")

	const { start_date, end_date, blocked, notes } = req.body as z.infer<typeof availabilityPatchSchema>
	...
	if (blocked) {
		...
		const existing = await prisma.blockedPeriod.findMany({ where: { property_id } })
		const toRemove = removableManualBlocks(start_date, end_date, existing.map(toPeriodInput))
		...
		if (idsToRemove.length > 0) {
			await prisma.blockedPeriod.deleteMany({ where: { id: { in: idsToRemove } } })
		}
	}
...
holdSlotsRouter.post("/", validate(holdSlotCreateSchema), async (req, res) => {
	...
	const { start_date, end_date } = req.body as z.infer<typeof holdSlotCreateSchema>
	const [blocks, holds] = await Promise.all([
		prisma.blockedPeriod.findMany({ where: { property_id } }),
		prisma.holdSlot.findMany({ where: { property_id } }),
	])

	if (!canCreateHold(start_date, end_date, blocks.map(toPeriodInput), holds.map(toHoldInput))) {
		res.status(409).json({ code: "dates_not_available", message: "The requested dates are not available" })
		return
	}
...
holdSlotsRouter.delete("/:hold_slot_id", async (req, res) => {
	const { property_id, hold_slot_id } = req.params
	const hold = await prisma.holdSlot.findUnique({ where: { id: hold_slot_id } })
