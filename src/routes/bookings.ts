...
import type { Booking, Client, Rate, DiscountRule } from "../../generated/prisma/client"
...
function serializeBooking(booking: Booking & { client: Client | null }) {
...
bookingsRouter.get("/", authenticate, async (req, res) => {
	...
	const managerProperties = await prisma.property.findMany({
		where: { user_id: req.manager.id, deleted_at: null },
		select: { id: true },
	})
	...
	if (req.query.start_date) where.check_in = { gte: new Date(req.query.start_date as string) }
	if (req.query.end_date) where.check_out = { lte: new Date(req.query.end_date as string) }
...
bookingsRouter.post("/public", validate(bookingCreateSchema), async (req, res) => {
	const body = req.body as z.infer<typeof bookingCreateSchema>

	const property = await prisma.property.findUnique({ where: { id: body.property_id } })
	if (!property || property.deleted_at !== null || !property.active) {
		throw new DomainError("not_found", "Property not found")
	}

	const now = new Date()
	const hold = await prisma.holdSlot.findUnique({ where: { id: body.hold_slot_id } })
	if (!hold || hold.property_id !== body.property_id) {
		res.status(409).json({ code: "hold_slot_expired", message: "No valid hold slot found for these dates" })
		return
	}
	if (hold.expires_at <= now) {
		res.status(409).json({ code: "hold_slot_expired", message: "Hold slot has expired" })
		return
	}
	if (
		...
		res.status(409).json({ code: "hold_slot_expired", message: "Hold slot dates do not match booking dates" })
		return
	}
	...
	const estimate = estimateStay(rates.map(toRateInput), body.check_in, body.check_out)
...
bookingsRouter.get("/:id", authenticate, async (req, res) => {
	...
	if (!booking.property || booking.property.user_id !== req.manager.id) {
...
bookingsRouter.patch("/:id", authenticate, validate(bookingUpdateSchema), async (req, res) => {
	const booking = await prisma.booking.findUnique({
		where: { id: req.params.id },
