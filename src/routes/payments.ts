...
paymentsRouter.get("/:booking_id/payments", authenticate, async (req, res, next) => {
	try {
		...
		if (booking.property_id) {
			const property = await prisma.property.findUnique({ where: { id: booking.property_id } })
...
webhookRouter.post(
	...
	async (req, res) => {
		...
		try {
			...
			res.status(500).json({ code: "webhook_error", message: "Internal error" })
			return
		}
