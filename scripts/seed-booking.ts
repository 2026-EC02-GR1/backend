...
async function main() {
	...
	if (!rate) {
		console.error(`No rate covers ${fmt(checkIn)} → ${fmt(checkOut)}. Run seed:property first.`)
		process.exit(1)
	}
	...
	console.log("Created hold slot:", hold.id, `(expires ${hold.expires_at.toISOString()})`)
	...
	console.log(`  Dates:  ${fmt(booking.check_in)} → ${fmt(booking.check_out)} (${nbNights} nights)`)
	console.log(`  Guest:  ${booking.client?.first_name} ${booking.client?.last_name}`)
